...
/// </summary>
internal sealed class UserPositionEntityConfiguration : IEntityTypeConfiguration<UserPosition>
{
    public void Configure(EntityTypeBuilder<UserPosition> builder)
    {
        builder.ToTable(
            name: $"{nameof(UserPosition)}s",
            buildAction: table => table.HasComment(comment: "Contain user position records.")
        );
