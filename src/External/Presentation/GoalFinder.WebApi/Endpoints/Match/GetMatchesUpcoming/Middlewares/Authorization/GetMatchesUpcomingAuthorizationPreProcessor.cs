...
namespace GoalFinder.WebApi.Endpoints.Match.GetMatchesUpcoming.Middlewares.Authorization;

/// <summary>
///     Pre-processor for <see cref="GetMatchesUpcomingRequest"/>
/// </summary>
internal sealed class GetMatchesUpcomingAuthorizationPreProcessor
    : PreProcessor<EmptyRequest, GetMatchesUpcomingStateBag>
{
    private readonly IServiceScopeFactory _serviceScopeFactory;
    private readonly TokenValidationParameters _tokenValidationParameters;

    public GetMatchesUpcomingAuthorizationPreProcessor(
    ...
    )
    {
        ...
        )
        {
            await SendResponseAsync(
                statusCode: GetMatchesUpcomingResponseStatusCode.FORBIDDEN,
                stateBag: state,
                context: context,
                ct: ct
            );
        ...
        var isRefreshTokenFound =
            await unitOfWork.GetMatchesUpcomingRepository.IsRefreshTokenFoundByAccessTokenIdQueryAsync(
        ...
        var isUserTemporarilyRemoved =
            await unitOfWork.GetMatchesUpcomingRepository.IsUserTemporarilyRemovedQueryAsync(
                userId: foundUser.Id,
                cancellationToken: ct
            );
    ...
    private static Task SendResponseAsync(
        GetMatchesUpcomingResponseStatusCode statusCode,
        GetMatchesUpcomingStateBag stateBag,
        IPreProcessorContext<EmptyRequest> context,
        CancellationToken ct
    )
    {
